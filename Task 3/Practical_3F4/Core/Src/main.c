...
int main(void)
{
  ...
  while (1)
  {
    ...
    /* USER CODE BEGIN 3 */
	  ...
	  StartClock = DWT_GetClock();
	  CheckSum = calculate_mandelbrot_double(Width, Height, MAX_ITER);
	  EndClock = DWT_GetClock();
	  ClockCycles = EndClock - StartClock;
	  ClockCycleSeconds = ((double)ClockCycles)/((double)SystemCoreClock);
	  Throughput = ((double)Width*(double)Height)/ClockCycleSeconds;
...
uint32_t DWT_GetClock(void){ //Get value of current clock cycle.
	return DWT->CYCCNT;
}
/* USER CODE BEGIN 4 */
