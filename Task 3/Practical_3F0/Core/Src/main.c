...
static uint32_t EndClock;
static uint32_t ClockCycles;
static double ClockCycleSeconds;
...
int main(void)
{
  ...
  //TODO: Initialize timer2 for clock cycle calculations, since there is no DWT in f0.
  Timer2_Init();
  /* USER CODE END Init */
  ...
  while (1)
  {
    ...
    /* USER CODE BEGIN 3 */
	  ...
	  StartClock = Timer2_Get();
	  CheckSum = calculate_mandelbrot_double(Width, Height, MAX_ITER);
	  EndClock = Timer2_Get();
	  ClockCycles = (EndClock >= StartClock) ? (EndClock - StartClock) : (0xFFFFFFFF - StartClock + EndClock); //CPU Clock cyles of mandelbrot calculation
	  ClockCycleSeconds = ((double)ClockCycles)/(48000000.0); // Divide clock cycle by clock frequency 48Mhz to get the cycles in seconds.
	  Throughput = ((double)Width*(double)Height)/ClockCycleSeconds; // Pixels generated per second = throughput
...
//TODO: Initialize method for TIM2
void Timer2_Init(void) {
    // Enable TIM2 clock
    RCC->APB1ENR |= RCC_APB1ENR_TIM2EN;

    TIM2->PSC = 0;              // Prescaler = 0 → tick at 48 MHz
    TIM2->ARR = 0xFFFFFFFF;     // Auto-reload: max
    TIM2->CNT = 0;              // Reset counter
    TIM2->CR1 |= TIM_CR1_CEN;   // Enable counter
}

uint32_t Timer2_Get(void) { //Get the current clock cycle
    return TIM2->CNT;
}
...
//TODO: Mandelbrot double version
uint64_t calculate_mandelbrot_double(int width, int height, int max_iterations) {
    uint64_t mandelbrot_sum = 0;
