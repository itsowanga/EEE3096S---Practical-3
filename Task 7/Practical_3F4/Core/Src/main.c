...
int main(void)
{
  ...
  while (1)
  {
    ...
    /* USER CODE BEGIN 3 */
	  ...
	  CheckSum = calculate_mandelbrot_fixed_point_arithmetic(Width,Height,MAX_ITER,1000);
	  EndTime = HAL_GetTick();
...
uint64_t calculate_mandelbrot_fixed_point_arithmetic(int width, int height, int max_iterations, int scale) {
    ...
    for (int y = 0; y < height; y++) {
        ...
        for (int x = 0; x < width; x++) {
            int64_t x0 = ((int64_t)x * (35 * scale) / (10 * width)) - (25 * scale / 10);
            int64_t xi = 0, yi = 0;
            int iteration = 0;

            while (iteration < max_iterations &&
                   ((xi * xi + yi * yi) <= (4LL * scale * scale))) {
                int64_t x_sq = (xi * xi) / scale;
                int64_t y_sq = (yi * yi) / scale;
                int64_t xy   = (xi * yi) / scale;
...
uint64_t calculate_mandelbrot_double(int width, int height, int max_iterations) {
    ...
    float inv_width  = 3.5 / width;
    float inv_height = 2.0 / height;

    for (int y = 0; y < height; y++) {
        float y0 = y * inv_height - 1.0;

        for (int x = 0; x < width; x++) {
            float x0 = x * inv_width - 2.5;
