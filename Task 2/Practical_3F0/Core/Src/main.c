...
uint32_t max_iter_array[] = {100,250,500,750,1000};
uint32_t image_size_array[] = {128,160,192,224,256};
...
int main(void)
{
  ...
  while (1)
  {
    ...
    /* USER CODE BEGIN 3 */
	  ...
	  uint16_t x = sizeof(max_iter_array)/sizeof(max_iter_array[0]);
	  uint16_t y = sizeof(image_size_array)/sizeof(image_size_array[0]);

	  for (uint32_t i = 0; i< x ;i++){
		  for (uint32_t z = 0; z< y ; z++){
			  volatile uint16_t size = image_size_array[z];
			  volatile uint16_t iter = max_iter_array[i];
			  StartTime = HAL_GetTick();
			  CheckSum = calculate_mandelbrot_fixed_point_arithmetic(size, size , iter);
			  EndTime = HAL_GetTick();
			  ExecutionTime = EndTime - StartTime;
			  total_cycles  = (uint64_t)ExecutionTime * (CPU_freq / 1000);
			  uint32_t pixels = image_size_array[z] * image_size_array[z];
